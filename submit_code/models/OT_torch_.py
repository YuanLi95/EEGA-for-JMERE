...
def GW_distance(X, Y, p, q,token_dependency_masks,image_rel_mask, lamda=0.5, iteration=5, OT_iteration=20):
	'''
	...
	Cs = torch.mul(Cs,image_rel_mask)
	print(Cs.shape)

	Ct = cos_batch_torch(Y, Y).float().cuda()
	Ct = torch.mul(Ct,token_dependency_masks)
	print(Ct.shape)
	# exit()
	# pdb.set_trace()
	bs = Cs.size(0)
...
def GW_distance_uniform(X, Y,token_dependency_masks,image_rel_mask, lamda=1e-1,iteration=5, OT_iteration=20):
	...
	distance,T = GW_distance(X, Y, p, q, token_dependency_masks, image_rel_mask, lamda=lamda, iteration=iteration,
